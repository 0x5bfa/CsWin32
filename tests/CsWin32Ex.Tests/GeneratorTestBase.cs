...
public abstract class GeneratorTestBase : IDisposable, IAsyncLifetime
{
    ...
    protected GeneratorManager CreateSuperGenerator(string[] metadataPaths, GeneratorOptions? options = null, CSharpCompilation? compilation = null, bool includeDocs = false) =>
        GeneratorManager.CreateFromGenerators(metadataPaths.Select(path => new Generator(path, includeDocs ? ApiDocumentationProvider.Get(ApiDocsPath) : null, [], options ?? DefaultTestGeneratorOptions, compilation ?? this.compilation, this.parseOptions)));
